 ...
 */
public class MorelOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "edu.ustb.sei.mde.morel.resource.morel.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "edu.ustb.sei.mde.morel.resource.morel.ui.occurences.declaration";
	...
	private edu.ustb.sei.mde.morel.resource.morel.ui.MorelTokenScanner tokenScanner;
	...
	private edu.ustb.sei.mde.morel.resource.morel.IMorelTextResource textResource;
	 ...
	 */
	public MorelOccurrence(edu.ustb.sei.mde.morel.resource.morel.IMorelTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, edu.ustb.sei.mde.morel.resource.morel.ui.MorelTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new edu.ustb.sei.mde.morel.resource.morel.mopp.MorelMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(edu.ustb.sei.mde.morel.resource.morel.ui.MorelBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, edu.ustb.sei.mde.morel.resource.morel.ui.MorelPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, edu.ustb.sei.mde.morel.resource.morel.ui.MorelPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, edu.ustb.sei.mde.morel.resource.morel.ui.MorelPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == edu.ustb.sei.mde.morel.resource.morel.ui.MorelPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(edu.ustb.sei.mde.morel.resource.morel.ui.MorelOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(edu.ustb.sei.mde.morel.resource.morel.ui.MorelOccurrence.DECLARATION_ANNOTATION_ID);
	}
