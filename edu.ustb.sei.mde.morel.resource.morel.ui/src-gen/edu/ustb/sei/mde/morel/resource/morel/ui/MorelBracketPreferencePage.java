 ...
 * </ul>
 */
public class MorelBracketPreferencePage extends org.eclipse.jface.preference.PreferencePage implements org.eclipse.ui.IWorkbenchPreferencePage {
	
	private static final String[] ALL_LEFT_BRACKETS = new String[] { "{", "(", "[", "<", "\"", "'", };
	private static final String[] ALL_RIGHT_BRACKETS = new String[] { "}", ")", "]", ">", "\"", "'", };
	
	private String BRACKETS_COLOR = edu.ustb.sei.mde.morel.resource.morel.ui.MorelPreferenceConstants.EDITOR_MATCHING_BRACKETS_COLOR;
	...
	private String language = new edu.ustb.sei.mde.morel.resource.morel.mopp.MorelMetaInformation().getSyntaxName();
	
	private edu.ustb.sei.mde.morel.resource.morel.ui.MorelBracketSet bracketsTmp;
	
	/**
	 * Creates a preference page for bracket setting.
	 */
	public MorelBracketPreferencePage() {
		super();
		
		edu.ustb.sei.mde.morel.resource.morel.IMorelMetaInformation metaInformation = new edu.ustb.sei.mde.morel.resource.morel.mopp.MorelMetaInformation();
	...
	public void init(org.eclipse.ui.IWorkbench workbench) {
		setPreferenceStore(edu.ustb.sei.mde.morel.resource.morel.ui.MorelUIPlugin.getDefault().getPreferenceStore());
		setDescription("Define the coloring of matching brackets.");
		
		bracketsTmp = new edu.ustb.sei.mde.morel.resource.morel.ui.MorelBracketSet(null, null);
		for (String languageID : languageIDs) {
			bracketSetTemp.put(languageID, getPreferenceStore().getString(languageID + edu.ustb.sei.mde.morel.resource.morel.ui.MorelPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		}
	}
	...
	private void handleMatchingBracketsSelection() {
		...
		enableCheckbox.setSelection(getPreferenceStore().getBoolean(		edu.ustb.sei.mde.morel.resource.morel.ui.MorelPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		enableClosingInside.setSelection(false);
		matchingBracketsColorButton.setEnabled(getPreferenceStore().getBoolean(		edu.ustb.sei.mde.morel.resource.morel.ui.MorelPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		...
		initializeLanguage();
		bracketsTmp.setBrackets(getPreferenceStore().getString(language + edu.ustb.sei.mde.morel.resource.morel.ui.MorelPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		...
		bracketsList.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int index : itemIndices) {
					edu.ustb.sei.mde.morel.resource.morel.IMorelBracketPair bracketPair = bracketsTmp.getBracketPair(index);
		...
		enableClosingInside.addSelectionListener(new org.eclipse.swt.events.SelectionListener() {
			...
			public void widgetSelected(org.eclipse.swt.events.SelectionEvent e) {
				...
				for (int idx : itemIndices) {
					edu.ustb.sei.mde.morel.resource.morel.IMorelBracketPair bracketPair = bracketsTmp.getBracketPair(idx);
	...
	protected void performDefaults() {
		enableCheckbox.setSelection(getPreferenceStore().getDefaultBoolean(edu.ustb.sei.mde.morel.resource.morel.ui.MorelPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX));
		matchingBracketsColorButton.setEnabled(enableCheckbox.getSelection());
		...
		bracketSetTemp.put(language, getPreferenceStore().getDefaultString(language + edu.ustb.sei.mde.morel.resource.morel.ui.MorelPreferenceConstants.EDITOR_BRACKETS_SUFFIX));
		bracketsTmp.setBrackets(bracketSetTemp.get(language));
		bracketsList.setItems(bracketsTmp.getBracketArray());
		enableClosingInside.setSelection(false);
	}
	...
	private void updateActiveEditor() {
		...
		getPreferenceStore().setValue(edu.ustb.sei.mde.morel.resource.morel.ui.MorelPreferenceConstants.EDITOR_MATCHING_BRACKETS_CHECKBOX, enableCheckbox.getSelection());
		getPreferenceStore().setValue(language + edu.ustb.sei.mde.morel.resource.morel.ui.MorelPreferenceConstants.EDITOR_BRACKETS_SUFFIX, bracketSetTemp.get(language));
		...
		if (editor != null && editor instanceof edu.ustb.sei.mde.morel.resource.morel.ui.MorelEditor) {
			((edu.ustb.sei.mde.morel.resource.morel.ui.MorelEditor) editor).invalidateTextRepresentation();
		}
	}
