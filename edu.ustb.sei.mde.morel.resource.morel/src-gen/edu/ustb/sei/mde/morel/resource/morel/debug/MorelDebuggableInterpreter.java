 ...
 */
public class MorelDebuggableInterpreter<ResultType, ContextType> extends edu.ustb.sei.mde.morel.resource.morel.debug.AbstractMorelDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private edu.ustb.sei.mde.morel.resource.morel.util.AbstractMorelInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private edu.ustb.sei.mde.morel.resource.morel.IMorelCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public MorelDebuggableInterpreter(edu.ustb.sei.mde.morel.resource.morel.util.AbstractMorelInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new edu.ustb.sei.mde.morel.resource.morel.IMorelInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = edu.ustb.sei.mde.morel.resource.morel.util.MorelStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public edu.ustb.sei.mde.morel.resource.morel.util.AbstractMorelInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		edu.ustb.sei.mde.morel.resource.morel.IMorelLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private edu.ustb.sei.mde.morel.resource.morel.IMorelLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		edu.ustb.sei.mde.morel.resource.morel.IMorelCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(edu.ustb.sei.mde.morel.resource.morel.debug.EMorelDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = edu.ustb.sei.mde.morel.resource.morel.util.MorelEObjectUtil.getDepth(current);
		stopCondition = new edu.ustb.sei.mde.morel.resource.morel.IMorelCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = edu.ustb.sei.mde.morel.resource.morel.util.MorelEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = edu.ustb.sei.mde.morel.resource.morel.util.MorelEObjectUtil.getDepth(current) - 1;
		stopCondition = new edu.ustb.sei.mde.morel.resource.morel.IMorelCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
