 ...
 */
public class MorelMinimalModelHelper {
	
	private final static edu.ustb.sei.mde.morel.resource.morel.util.MorelEClassUtil eClassUtil = new edu.ustb.sei.mde.morel.resource.morel.util.MorelEClassUtil();
			...
			if (feature instanceof org.eclipse.emf.ecore.EReference) {
				...
				if (type instanceof org.eclipse.emf.ecore.EClass) {
					...
					for (int i = 0; i < lowerBound; i++) {
						...
						if (reference.isContainment()) {
							...
							String initialValue = "#some" + edu.ustb.sei.mde.morel.resource.morel.util.MorelStringUtil.capitalize(typeClass.getName());
						...
						if (value instanceof java.util.List<?>) {
							java.util.List<org.eclipse.emf.ecore.EObject> list = edu.ustb.sei.mde.morel.resource.morel.util.MorelListUtil.castListUnchecked(value);
							list.add(subModel);
						} else {
				...
				if ("EString".equals(attribute.getEType().getName())) {
					...
					if (attribute.getName().equals("name") && name != null) {
						...
						initialValue = "some" + edu.ustb.sei.mde.morel.resource.morel.util.MorelStringUtil.capitalize(attribute.getName());
					}
					Object value = root.eGet(attribute);
					if (value instanceof java.util.List<?>) {
						java.util.List<String> list = edu.ustb.sei.mde.morel.resource.morel.util.MorelListUtil.castListUnchecked(value);
