...
public class MorelDefaultNameProvider implements edu.ustb.sei.mde.morel.resource.morel.IMorelNameProvider {
		...
		if (nameAttr instanceof org.eclipse.emf.ecore.EAttribute) {
			...
			for (org.eclipse.emf.ecore.EOperation operation : element.eClass().getEAllOperations()) {
				if (operation.getName().toLowerCase().endsWith(NAME_FEATURE) && operation.getEParameters().size() == 0) {
					Object result = edu.ustb.sei.mde.morel.resource.morel.util.MorelEObjectUtil.invokeOperation(element, operation);
					if (result != null) {
