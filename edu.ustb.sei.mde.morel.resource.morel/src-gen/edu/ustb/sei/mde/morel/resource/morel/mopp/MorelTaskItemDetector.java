...
/**
 * The MorelTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * MorelTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class MorelTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<edu.ustb.sei.mde.morel.resource.morel.mopp.MorelTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<edu.ustb.sei.mde.morel.resource.morel.mopp.MorelTaskItem> foundItems = new java.util.ArrayList<edu.ustb.sei.mde.morel.resource.morel.mopp.MorelTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new edu.ustb.sei.mde.morel.resource.morel.mopp.MorelTaskItem(keyword, message, localLine, offset, end));
