...
//								ValueNode target = e.getTarget();
	...
	public static TypedGraph prepareCon2(TypedGraph baseGraph, TypedGraph... branchGraphs)
			throws NothingReturnedException {
				...
				public void run() {
					...
					for (TypedNode n : branchTypeNodeMap.get(typeNode)) {
						try {
							...
							TypedNode rn = null;
							try {
								// 根据n的索引查找result图中有无相应的对象，如果有就赋值给rn。说明不同分支图添加了相同的元素
								rn = resultGraph.getElementByIndexObject(n.getIndex());
							} catch (NothingReturnedException e1) { // 如果result图中无相应的对象，则将n对象添加到result图的List<TypedNode>，并且令rn=null
								resultGraph.addTypedNode(n);
								rn = null;
					...
					System.out.println("*****新加 "+ typeNode + "TypedNodes: " + Profiler.end() + "ms");
				}
			});
			addTypedNodesMap.put(typeNode, thread);
			thread.start();
		...
		HashMap<TypedNode, TypedNode> changeTypedNodesMap = new HashMap<>();
		HashMap<TypeNode, Thread> deleteAndChangeTypedNodesMap = new HashMap<>();
		HashMap<TypeNode, ArrayList<TypedNode>> deleteTypedNodesMap = new HashMap<>();
		for (int i = 0; i < allTypeNodes.size(); i++) {
			TypeNode typeNode = allTypeNodes.get(i);
			ArrayList<TypedNode> deleteTypedNodesList = new ArrayList<>();
			deleteTypedNodesMap.put(typeNode, deleteTypedNodesList);
		...
		// 删除和修改TypedEdges
		HashMap<TypeEdge, Thread> deleteAndChangeTypedEdgesMap = new HashMap<>();
		Profiler.begin();
		for (int i = 0; i < allTypeEdges.size(); i++) {

			TypeEdge typeEdge = allTypeEdges.get(i);

			Thread thread = new Thread(new Runnable() {
				@Override
				public void run() {
					try {
						deleteAndChangeTypedNodesMap.get(typeEdge.getSource()).join();
						deleteAndChangeTypedNodesMap.get(typeEdge.getTarget()).join();
					} catch (InterruptedException e1) {
					}

					TypedEdge[] typedEdgeImages = new TypedEdge[branchGraphs.length];
					for (TypedEdge baseEdge : baseTypeEdgeMap.get(typeEdge)) {
			...
			deleteAndChangeTypedEdgesMap.put(typeEdge, thread);
			thread.start();
		}
		System.out.println("*****删除和修改TypedEdges: " + Profiler.end() + "ms");
		...
		// 删除和修改ValueEdges
		HashMap<PropertyEdge, Thread> deleteAndChangeValueEdges = new HashMap<>();
		Profiler.begin();
		for (int i = 0; i < allPropertyEdges.size(); i++) {
			ValueEdge[] valueEdgeImages = new ValueEdge[branchGraphs.length];

			PropertyEdge propertyEdge = allPropertyEdges.get(i);

			Thread thread = new Thread(new Runnable() {
				@Override
				public void run() {
					try {
						deleteAndChangeTypedEdgesMap.get(propertyEdge.getSource()).join();
						addValueNodesThread.join();
					} catch (InterruptedException e1) {
					}

					for (ValueEdge baseEdge : basePropertyEdgeMap.get(propertyEdge)) { // 对于基本图中每一个条ValueEdge类型的边
			...
			deleteAndChangeValueEdges.put(propertyEdge, thread);
			thread.start();
		}
		System.out.println("*******删除和修改ValueEdges：" + Profiler.end() + "ms");
		...
		for (int i = 0; i < allTypeNodes.size(); i++) {
			...
			Thread thread = new Thread(new Runnable() {
				...
				public void run() {
					try {
						deleteAndChangeTypedNodesMap.get(typeNode).join();
					} catch (InterruptedException e1) {
					}

					deleteTypedNodesMap.get(typeNode).forEach(n -> {
						// 如果此节点还是某条TypedEdge类型边e的source或target端点，删除这条边e
						for (int i = resultGraph.allTypedEdges.size() - 1; i >= 0; i--) {
							TypedEdge e = resultGraph.allTypedEdges.get(i);
							if (resultGraph.isEqual(e.getSource(), n) || resultGraph.isEqual(e.getTarget(), n)) {
