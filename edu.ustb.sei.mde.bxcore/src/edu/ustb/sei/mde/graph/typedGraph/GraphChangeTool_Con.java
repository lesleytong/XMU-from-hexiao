...
public class GraphChangeTool_Con {
	...
	// 交换序，删y个
	public static void changeTypedEdges(ConcurrentTypedGraph graph, int x, int y) {
		...
		if (changeTypedEdgesFlag == true) {
			...
			TypeEdge type = graph.getConcurrentTypeGraph().getTypeEdge(graph.getConcurrentTypeGraph().getTypeNode("A"), "a2b");
			...
			if(helpFlag == true) {
				...
				TypeNode typeB = graph.getConcurrentTypeGraph().getTypeNode("B");
			...
			TypeEdge type = graph.getConcurrentTypeGraph().getTypeEdge(graph.getConcurrentTypeGraph().getTypeNode("C"), "c2d");
			TypedEdge e = new TypedEdge();
			TypedEdge er = null;	
			boolean helpFlag = false;
			for (int i = 0; i < graph.getAllTypedEdges().size(); i++) {
			...
			if(helpFlag == true) {
				...
				TypeNode typeD = graph.getConcurrentTypeGraph().getTypeNode("D");
				for (int i = 0; i < graph.getAllTypedNodes().size(); i++) {
					TypedNode n = graph.getAllTypedNodes().get(i);
...
//		System.out.println(graph.getAllTypedEdges());
	...
	// 删y个
	public static void changeValueEdges(ConcurrentTypedGraph graph, int y) {
...
//		System.out.println(graph.getAllValueEdges());
	...
	// 改x个，删y个
	public static void changeTypedNodes(ConcurrentTypedGraph graph, int x, int y) {
...
//		System.out.println(graph.getAllValueNodes());
	...
	public static void addTypedNodesAndTypedEdges(ConcurrentTypedGraph graph, int z) {
...
//		System.out.println("添加TypedEdges后的TypedNodes的规模：" + graph.getAllTypedNodes().size());
	...
	public static void addNodesAndValueEdges(ConcurrentTypedGraph graph, int z) {
