...
protected GraphModel getSolverModel(TypedGraph typedGraph, Context base) {
	...
	public INode getNode(String name) {
		return this.nodes.stream()
				.filter(n->name.equals(((INamedElement)n).getName()))
				.findFirst()
				.get();
	}
	...
	public TypedGraph construct(TypedGraph referenceGraph, Context context) throws UninitializedException, NothingReturnedException {
			...
			if(n instanceof PatternEdge) {
				...
				try {
					...
					edge = creator.createTypedEdge(((PatternElement<?>)n.getSource()).getName(), 
							((PatternElement<?>)n.getTarget()).getName(), 
							((PatternEdge) n).getType(),index);
				}
				...
				try {
					...
					edge = creator.createValueEdge(((PatternElement<?>)n.getSource()).getName(), 
							((PatternElement<?>)n.getTarget()).getName(), 
							((PatternValueEdge) n).getType(), index);
				}
			}
		}
	...
	public ContextType getType() {
		if(type==null) {
			...
			this.getNodes().forEach(n->{type.addField(((PatternElement<?>)n).getName(), ((PatternElement<?>)n).getType());});
			this.getEdges().forEach(e->{type.addField(((PatternElement<?>)e).getName(), ((PatternElement<?>)e).getType());});
		}
		return type;
	}
	...
	public void declare(String patternShape) {
		...
		String edgeDecl = "\\s*(\\w+)\\s*:\\s*(\\w+)\\s*\\-(\\w+)->\\s*(\\w+)\\s*";
		...
		for(String stat : statements) {
			...
			if((matcher = nodePat.matcher(stat)).matches()) {
				...
				String targetName = matcher.group(4);
