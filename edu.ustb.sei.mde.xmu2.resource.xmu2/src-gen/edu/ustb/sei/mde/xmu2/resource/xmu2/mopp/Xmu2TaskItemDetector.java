...
/**
 * The Xmu2TaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * Xmu2TaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class Xmu2TaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<edu.ustb.sei.mde.xmu2.resource.xmu2.mopp.Xmu2TaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<edu.ustb.sei.mde.xmu2.resource.xmu2.mopp.Xmu2TaskItem> foundItems = new java.util.ArrayList<edu.ustb.sei.mde.xmu2.resource.xmu2.mopp.Xmu2TaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new edu.ustb.sei.mde.xmu2.resource.xmu2.mopp.Xmu2TaskItem(keyword, message, localLine, offset, end));
