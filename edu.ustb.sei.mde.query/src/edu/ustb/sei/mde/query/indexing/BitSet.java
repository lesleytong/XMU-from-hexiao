...
public class BitSet implements Cloneable, java.io.Serializable {
    ...
    */
     ...
     * 
     * @param set
     * @return <code>true</code> if all bits in <code>set</code> are also set in this
     */
    public boolean cover(BitSet set) {
    	if (this == set)
            return true;
    	
    	int minWordInUse = Math.min(wordsInUse, set.wordsInUse);
    	
    	for(int i=0; i<minWordInUse; i++ ) {
    		if(Long.compareUnsigned(words[i], set.words[i])<0) return false;
    	}
    	
    	for(int i=minWordInUse;i<set.wordsInUse;i++)
    		if(set.words[i]!=0) return false;
