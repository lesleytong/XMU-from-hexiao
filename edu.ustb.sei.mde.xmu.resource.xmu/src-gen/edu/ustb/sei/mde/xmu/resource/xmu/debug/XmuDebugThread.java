...
public class XmuDebugThread extends edu.ustb.sei.mde.xmu.resource.xmu.debug.XmuDebugElement implements org.eclipse.debug.core.model.IThread, edu.ustb.sei.mde.xmu.resource.xmu.debug.IXmuDebugEventListener {
	
	private boolean suspended = false;
	private edu.ustb.sei.mde.xmu.resource.xmu.debug.XmuDebugTarget debugTarget;
	
	public XmuDebugThread(edu.ustb.sei.mde.xmu.resource.xmu.debug.XmuDebugTarget target) {
		super(target);
	...
	public org.eclipse.debug.core.model.IStackFrame[] getStackFrames() throws org.eclipse.debug.core.DebugException {
		if (isSuspended()) {
			edu.ustb.sei.mde.xmu.resource.xmu.debug.XmuDebugMessage stack = this.debugTarget.getDebugProxy().getStack();
			String framesData = stack.getArgument(0);
			if (framesData != null && !"".equals(framesData)) {
				java.util.List<String> frames = edu.ustb.sei.mde.xmu.resource.xmu.util.XmuStringUtil.decode(framesData, '#');
				...
				for (int i = 0; i < frames.size(); i++) {
					...
					theFrames[frames.size() - i - 1] = new edu.ustb.sei.mde.xmu.resource.xmu.debug.XmuStackFrame(getTarget(), data);
				}
				return theFrames;
			}
		}
	...
	public edu.ustb.sei.mde.xmu.resource.xmu.debug.XmuDebugTarget getTarget() {
		return debugTarget;
	}
