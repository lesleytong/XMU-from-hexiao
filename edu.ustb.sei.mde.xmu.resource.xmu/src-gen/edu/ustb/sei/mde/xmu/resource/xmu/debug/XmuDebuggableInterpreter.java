 ...
 */
public class XmuDebuggableInterpreter<ResultType, ContextType> extends edu.ustb.sei.mde.xmu.resource.xmu.debug.AbstractXmuDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private edu.ustb.sei.mde.xmu.resource.xmu.util.AbstractXmuInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private edu.ustb.sei.mde.xmu.resource.xmu.IXmuCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public XmuDebuggableInterpreter(edu.ustb.sei.mde.xmu.resource.xmu.util.AbstractXmuInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new edu.ustb.sei.mde.xmu.resource.xmu.IXmuInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = edu.ustb.sei.mde.xmu.resource.xmu.util.XmuStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public edu.ustb.sei.mde.xmu.resource.xmu.util.AbstractXmuInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		edu.ustb.sei.mde.xmu.resource.xmu.IXmuLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private edu.ustb.sei.mde.xmu.resource.xmu.IXmuLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		edu.ustb.sei.mde.xmu.resource.xmu.IXmuCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(edu.ustb.sei.mde.xmu.resource.xmu.debug.EXmuDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = edu.ustb.sei.mde.xmu.resource.xmu.util.XmuEObjectUtil.getDepth(current);
		stopCondition = new edu.ustb.sei.mde.xmu.resource.xmu.IXmuCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = edu.ustb.sei.mde.xmu.resource.xmu.util.XmuEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = edu.ustb.sei.mde.xmu.resource.xmu.util.XmuEObjectUtil.getDepth(current) - 1;
		stopCondition = new edu.ustb.sei.mde.xmu.resource.xmu.IXmuCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
