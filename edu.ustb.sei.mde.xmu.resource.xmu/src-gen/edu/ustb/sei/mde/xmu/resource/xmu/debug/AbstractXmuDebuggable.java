...
public abstract class AbstractXmuDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<edu.ustb.sei.mde.xmu.resource.xmu.util.XmuPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<edu.ustb.sei.mde.xmu.resource.xmu.util.XmuPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new edu.ustb.sei.mde.xmu.resource.xmu.util.XmuRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new edu.ustb.sei.mde.xmu.resource.xmu.util.XmuRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(edu.ustb.sei.mde.xmu.resource.xmu.debug.EXmuDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			edu.ustb.sei.mde.xmu.resource.xmu.debug.XmuDebugMessage message = new edu.ustb.sei.mde.xmu.resource.xmu.debug.XmuDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				edu.ustb.sei.mde.xmu.resource.xmu.util.XmuPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(edu.ustb.sei.mde.xmu.resource.xmu.debug.EXmuDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(edu.ustb.sei.mde.xmu.resource.xmu.debug.EXmuDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new edu.ustb.sei.mde.xmu.resource.xmu.util.XmuPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new edu.ustb.sei.mde.xmu.resource.xmu.util.XmuPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(edu.ustb.sei.mde.xmu.resource.xmu.debug.EXmuDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(edu.ustb.sei.mde.xmu.resource.xmu.debug.EXmuDebugMessageTypes.STARTED, true);
			setSuspend(true);
