...
/**
 * The XmuTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * XmuTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class XmuTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<edu.ustb.sei.mde.xmu.resource.xmu.mopp.XmuTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<edu.ustb.sei.mde.xmu.resource.xmu.mopp.XmuTaskItem> foundItems = new java.util.ArrayList<edu.ustb.sei.mde.xmu.resource.xmu.mopp.XmuTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new edu.ustb.sei.mde.xmu.resource.xmu.mopp.XmuTaskItem(keyword, message, localLine, offset, end));
