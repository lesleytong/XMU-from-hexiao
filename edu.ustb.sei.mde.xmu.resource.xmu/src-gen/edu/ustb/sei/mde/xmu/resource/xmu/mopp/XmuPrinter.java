...
public class XmuPrinter implements edu.ustb.sei.mde.xmu.resource.xmu.IXmuTextPrinter {
	...
	protected void doPrint(org.eclipse.emf.ecore.EObject element, java.io.PrintWriter out, String globaltab) {
		...
		if (element instanceof edu.ustb.sei.mde.xmu.ResetStatement) {
			print_edu_ustb_sei_mde_xmu_ResetStatement((edu.ustb.sei.mde.xmu.ResetStatement) element, globaltab, out);
			return;
		}
	...
	public void print_edu_ustb_sei_mde_xmu_ResetStatement(edu.ustb.sei.mde.xmu.ResetStatement element, String outertab, java.io.PrintWriter out) {
		String localtab = outertab;
		// The printCountingMap contains a mapping from feature names to the number of
		// remaining elements that still need to be printed. The map is initialized with
		// the number of elements stored in each structural feature. For lists this is the
		...
		java.util.Map<String, Integer> printCountingMap = new java.util.LinkedHashMap<String, Integer>(1);
		Object temp;
		temp = element.eGet(element.eClass().getEStructuralFeature(edu.ustb.sei.mde.xmu.XmuPackage.RESET_STATEMENT__VARIABLES));
		...
		out1 = new java.io.PrintWriter(sWriter);
		printCountingMap1 = new java.util.LinkedHashMap<String, Integer>(printCountingMap);
		print_edu_ustb_sei_mde_xmu_ResetStatement_0(element, localtab, out1, printCountingMap1);
		if (printCountingMap.equals(printCountingMap1)) {
			out1.close();
		} else {
			out1.flush();
	...
	public void print_edu_ustb_sei_mde_xmu_ResetStatement_0(edu.ustb.sei.mde.xmu.ResetStatement element, String outertab, java.io.PrintWriter out, java.util.Map<String, Integer> printCountingMap) {
		String localtab = outertab;
		int count;
		boolean iterate = true;
		java.io.StringWriter sWriter = null;
		...
		if (count > 0) {
			Object o = element.eGet(element.eClass().getEStructuralFeature(edu.ustb.sei.mde.xmu.XmuPackage.RESET_STATEMENT__VARIABLES));
			java.util.List<?> list = (java.util.List<?>) o;
			int index = list.size() - count;
			if (index >= 0) {
				o = list.get(index);
			...
			if (o != null) {
				edu.ustb.sei.mde.xmu.resource.xmu.IXmuTokenResolver resolver = tokenResolverFactory.createTokenResolver("NAME");
				resolver.setOptions(getOptions());
				out.print(resolver.deResolve(getReferenceResolverSwitch() == null ? null : getReferenceResolverSwitch().getResetStatementVariablesReferenceResolver().deResolve((edu.ustb.sei.mde.xmu.Variable) o, element, (org.eclipse.emf.ecore.EReference) element.eClass().getEStructuralFeature(edu.ustb.sei.mde.xmu.XmuPackage.RESET_STATEMENT__VARIABLES)), element.eClass().getEStructuralFeature(edu.ustb.sei.mde.xmu.XmuPackage.RESET_STATEMENT__VARIABLES), element));
				out.print(" ");
			}
		...
		while (iterate) {
			...
			out1 = new java.io.PrintWriter(sWriter);
			printCountingMap1 = new java.util.LinkedHashMap<String, Integer>(printCountingMap);
			print_edu_ustb_sei_mde_xmu_ResetStatement_0_0(element, localtab, out1, printCountingMap1);
			if (printCountingMap.equals(printCountingMap1)) {
				iterate = false;
				out1.close();
			} else {
	...
	public void print_edu_ustb_sei_mde_xmu_ResetStatement_0_0(edu.ustb.sei.mde.xmu.ResetStatement element, String outertab, java.io.PrintWriter out, java.util.Map<String, Integer> printCountingMap) {
		int count;
		// DEFINITION PART BEGINS (CsString)
		out.print(",");
		out.print(" ");
