 ...
 */
public class XmuOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "edu.ustb.sei.mde.xmu.resource.xmu.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "edu.ustb.sei.mde.xmu.resource.xmu.ui.occurences.declaration";
	...
	private edu.ustb.sei.mde.xmu.resource.xmu.ui.XmuTokenScanner tokenScanner;
	...
	private edu.ustb.sei.mde.xmu.resource.xmu.IXmuTextResource textResource;
	 ...
	 */
	public XmuOccurrence(edu.ustb.sei.mde.xmu.resource.xmu.IXmuTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, edu.ustb.sei.mde.xmu.resource.xmu.ui.XmuTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new edu.ustb.sei.mde.xmu.resource.xmu.mopp.XmuMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(edu.ustb.sei.mde.xmu.resource.xmu.ui.XmuBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, edu.ustb.sei.mde.xmu.resource.xmu.ui.XmuPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, edu.ustb.sei.mde.xmu.resource.xmu.ui.XmuPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, edu.ustb.sei.mde.xmu.resource.xmu.ui.XmuPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == edu.ustb.sei.mde.xmu.resource.xmu.ui.XmuPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(edu.ustb.sei.mde.xmu.resource.xmu.ui.XmuOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(edu.ustb.sei.mde.xmu.resource.xmu.ui.XmuOccurrence.DECLARATION_ANNOTATION_ID);
	}
